...
import com.lychee.sely.adstv.ads.image.ImageAdFragment;
...
public class ScheduledAdImageActivity extends FragmentActivity {

    Button mbBrowseImage;
    ImageCardView micvImage;

    private long mProgramID;
    private long mScheduledAdItemID;
    private long mAdID;
    private Button mbCreateScheduledAdImage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_scheduled_ad_image);
        ...
        mtvAdName = (TextView) findViewById(R.id.text_view_ad_name_scheduled_ad_image);

        micvImage = (ImageCardView) findViewById(R.id.image_card_view_image);

        mbBrowseImage = (Button) findViewById(R.id.browse_scheduled_ad_image);
        mbCreateScheduledAdImage = (Button) findViewById(R.id.create_scheduled_ad_image);
        ...
        mbBrowseImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                startActivityForResult(i, ImageAdFragment.AD_SELECTED);
            }
        });

        mbCreateScheduledAdImage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {


                if (!(mAdID > -1)) {
                    Toast.makeText(getBaseContext(), "Image Ad is not selected.", Toast.LENGTH_SHORT).show();
                    mbBrowseImage.requestFocus();
                    return;
                }
                ...
                Cursor cImageAd = getContentResolver().query(ProgramAdContentProvider.IMAGE_AD_URI, null, "_ID=?", new String[]{mAdID + ""}, null);
                if (cImageAd != null && cImageAd.getCount() > 0) {
                    cImageAd.moveToFirst();
                    String imagePath = cImageAd.getString(cImageAd.getColumnIndex("AdPath"));
                    cImageAd.close();
                    //get serial no
                    if (mScheduledAdItemID == -1) {
                        ...
                        cv.put("AdType", ScheduledAd.AD_TYPE_IMAGE);
                        cv.put("AdCode", mAdID);
                        cv.put("AdName", mtvAdName.getText().toString().trim());
                        cv.put("AdPath", imagePath);
        ...
        if (mScheduledAdItemID > 0) {
            ...
            if (c != null && c.getCount() > 0) {
                ...
                String imagePath = c.getString(c.getColumnIndex("AdPath"));
                String sDateTime = c.getString(c.getColumnIndex("AdDateTime"));
                c.close();

                Drawable dImage = FileOperation.imgDrawableFromFile(Resources.getSystem(), imagePath);
                micvImage.setMainImageAdjustViewBounds(true);
                micvImage.setMainImageScaleType(ImageView.ScaleType.CENTER_INSIDE);
                micvImage.setMainImage(dImage);
    ...
    private void setNavigation(){
        //Default
        mbBrowseImage.setNextFocusRightId(R.id.number_picker_year);
        mbBrowseImage.setNextFocusDownId(R.id.create_scheduled_ad_image);
        mnpYear.setNextFocusLeftId(R.id.browse_scheduled_ad_image);
        ...
        mnpSeconds.setNextFocusRightId(R.id.create_scheduled_ad_image);
        mbCreateScheduledAdImage.setNextFocusUpId(R.id.number_picker_seconds);
        mbCreateScheduledAdImage.setNextFocusLeftId(R.id.browse_scheduled_ad_image);

        if(mRepeatMask==ScheduledAdProgram.REPEAT_TYPE_YEARLY){
            mnpYear.setVisibility(View.GONE);

            mbBrowseImage.setNextFocusRightId(R.id.number_picker_month);
            mbBrowseImage.setNextFocusDownId(R.id.create_scheduled_ad_image);
            mnpMonth.setNextFocusLeftId(R.id.browse_scheduled_ad_image);
            mnpMonth.setNextFocusRightId(R.id.number_picker_day);
            mnpDay.setNextFocusLeftId(R.id.number_picker_month);
            mnpDay.setNextFocusRightId(R.id.number_picker_hours);
            mnpHours.setNextFocusLeftId(R.id.number_picker_day);
            ...
            mbBrowseImage.setNextFocusRightId(R.id.number_picker_day);
            mbBrowseImage.setNextFocusDownId(R.id.create_scheduled_ad_image);
            mnpDay.setNextFocusLeftId(R.id.browse_scheduled_ad_image);
            mnpDay.setNextFocusRightId(R.id.number_picker_hours);
            mnpHours.setNextFocusLeftId(R.id.number_picker_day);
            mnpHours.setNextFocusRightId(R.id.number_picker_minutes);
            mnpMinutes.setNextFocusLeftId(R.id.number_picker_hours);
            ...
            mbBrowseImage.setNextFocusRightId(R.id.number_picker_hours);
            mbBrowseImage.setNextFocusDownId(R.id.create_scheduled_ad_image);
            mnpHours.setNextFocusLeftId(R.id.browse_scheduled_ad_image);
            mnpHours.setNextFocusRightId(R.id.number_picker_minutes);
            mnpMinutes.setNextFocusLeftId(R.id.number_picker_hours);
            mnpMinutes.setNextFocusRightId(R.id.number_picker_seconds);
            mnpSeconds.setNextFocusLeftId(R.id.number_picker_minutes);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageAdFragment.AD_SELECTED && resultCode == Activity.RESULT_OK) {
            mAdID = data.getLongExtra("AD_ID", 0);
            //Update the image with new one
            Cursor c = getContentResolver().query(ProgramAdContentProvider.IMAGE_AD_URI, null, "_ID=?", new String[]{mAdID + ""}, null);
            if (c != null && c.getCount() > 0) {
                c.moveToFirst();
                mtvAdName.setText(c.getString(c.getColumnIndex("AdName")));
                String imagePath = c.getString(c.getColumnIndex("AdPath"));
