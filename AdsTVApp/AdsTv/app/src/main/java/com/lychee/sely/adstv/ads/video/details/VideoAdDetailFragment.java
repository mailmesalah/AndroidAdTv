...
public class VideoAdDetailFragment extends DetailsFragment {
    private static final String TAG = VideoAdDetailFragment.class.getSimpleName();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAd=new Ad();
        Cursor c = getActivity().getContentResolver().query(ProgramAdContentProvider.VIDEO_AD_URI, null, "_ID=?", new String[]{mAdID + ""}, null);
        if(c!=null&&c.getCount()>0){
            c.moveToFirst();
            mAd.setId(mAdID);
            mAd.setAdPath(c.getString(c.getColumnIndex("AdPath")));
            c.close();
        }
    ...
    private void setupFragment(){
        ...
        FullWidthDetailsOverviewRowPresenter rowPresenter = new FullWidthDetailsOverviewRowPresenter(new VideoAdDetailPresenter());
            ...
            public void onActionClicked(Action action) {
                if(action.getId()==ACTION_SELECT){
                    ...
                    if(c!=null && c.getCount()>0){
                        ...
                        getActivity().getContentResolver().delete(ProgramAdContentProvider.VIDEO_AD_URI,"_ID=?",new String[]{mAdID+""});
                    }
